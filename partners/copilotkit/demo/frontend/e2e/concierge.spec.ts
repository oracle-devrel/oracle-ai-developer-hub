...
test.describe("Travel Concierge · Oracle Agent Spec × Memory", () => {
  ...
  test("recalls a preference in a brand-new session (cross-session memory)", async ({
    ...
    // ── Session A — store a unique fact. The concierge persists the turn as the
    // stream drains, then Oracle Agent Memory extracts + embeds + indexes it
    // asynchronously, so the fact is not instantly recallable.
    await openChat(page);
